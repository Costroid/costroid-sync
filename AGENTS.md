...
T1.5 Braille/Dot Interaction Polish — IMPLEMENTED (2026-05-30). A visual/interaction polish pass over the existing opt-in `tui` and `sync --tui` surfaces ONLY, expressing the approved dot/braille identity (braille mark + wordmark header, filled/hollow dot panel selection, braille/block spend meters, a static recent-spend sparkline, dotted separators, real-state sync dot-progress). Constraints: no new Go dependency (stays within ADR-011 scope); no provider/storage/cloud/default-output change; metadata-only; money never animated; selection legible with zero color; full ASCII/`NO_COLOR`/`--plain`/non-TTY fallback preserved. It is still opt-in and never default CLI behavior. NOTE: the original "one accent color / monochrome-first" framing in this entry is superseded by the founder-approved T1.6 cold/warm color identity below — ONLY the color-count rule changed; pure-monochrome legibility and every other constraint here are fully retained.

T1.6 CLI-cold/SYNC-warm Color Identity — IMPLEMENTED (2026-05-31, founder-approved). A visual/interaction polish pass over the two existing opt-in TUI surfaces ONLY, giving each a distinct color identity from the brand ramps. This is a deliberate, founder-approved override of the T1.5 "one accent color / monochrome-first" rule — and ONLY of the color-count rule; every other T1.2/T1.3/T1.5 safety invariant is retained.
- Surfaces: the dashboard (bare `costroid`, the CLI surface) uses the COLD cyan-blue ramp (`#042C53 #185FA5 #378ADD #85B7EB`); `costroid sync --tui` uses the WARM coral-amber ramp (`#712B13 #D85A30 #F0997B #F5C4B3`). Signal green `#C8FF3D` stays the single shared accent (primary money / brand mark), identical on both surfaces; the ash faint (`#888780`) and alert red monotones are shared. Severity 0→8 (idle→critical) escalates as braille/dot density in the surface ramp hue, with the over/critical tier using the alert (red) treatment.
- Graceful degradation is mandatory and explicit: `tui.ResolveTier` maps `COLORTERM`/`TERM` (and `--plain`/`NO_COLOR`) to truecolor → 256 → ANSI-16 → monochrome, choosing hand-picked per-tier values (cold→blue/cyan, warm→yellow, signal→green, faint→bright-black, alert→red in 16-color). The hierarchy MUST still read in 16-color AND in pure monochrome (carried by glyph shape + text + position), so color is never the sole signal: over-budget, anomalies/severity, and panel selection all read with color stripped (filled vs hollow dot, `OVER`/`ALERT` markers, braille dot density).
- Retained invariants (unchanged from T1.2/T1.3/T1.5): metadata-only; read-only over local SQLite with no network/provider/credential access from the dashboard (`sync --tui` still calls providers only because the user explicitly ran `sync`); no new Go dependency (stays within ADR-011 scope — `bubbletea v1.3.10` + scoped `bubbles v0.21.0`, no new bubbles component); money never animated and never partially rendered; animation only reflects real sync-task state; full ASCII/`NO_COLOR`/`--plain`/non-UTF-8 fallback and the non-TTY/CI/`TERM=dumb` behavior (dashboard prints help; `sync --tui` falls back to plain `sync`) preserved — never paint an alternate screen into a pipe. Still opt-in surfaces; the dashboard-as-default decision (T1.2) is unchanged. The `tui/` package keeps passing `imports_test.go` (no `providers`/`client`/`net`/`net/http`/`os/exec`).
